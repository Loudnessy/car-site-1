...
export const Modal = ({modalIsOpen, closeModal}) => {
  ...
  >
    ...
    <StyledModalContainer>
      ...
      <StyledModalHeader>
        {selectedCar?.make} <span>{selectedCar?.model}</span>, {selectedCar?.year}
      </StyledModalHeader>
      {/* <p>
      ...
      <StyledModalTagsList>
        <StyledModalTagsListItem><StyledModalTag>{selectedCar?.address.split(' ').slice(-1).join(' ').replace(',', '')}</StyledModalTag>
        <svg width="2" height="16">
<use href={sprite + "#stick"} />
</svg>
        </StyledModalTagsListItem>
        <StyledModalTagsListItem><StyledModalTag>{selectedCar?.address.split(' ').slice(3, 4).join(' ').replace(',', '')}</StyledModalTag>
        <svg width="2" height="16">
<use href={sprite + "#stick"} />
</svg>
        </StyledModalTagsListItem>

        <StyledModalTagsListItem><StyledModalTag>Id: {selectedCar?.id.toString()}</StyledModalTag>
        <svg width="2" height="16">
<use href={sprite + "#stick"} />
</svg>
        </StyledModalTagsListItem>
...
<use href={sprite + "#stick"} />
        ...
        <StyledModalTagsListItem><StyledModalTag>Type: {selectedCar?.type}</StyledModalTag>
        <svg width="2" height="16">
<use href={sprite + "#stick"} />
</svg>
        </StyledModalTagsListItem>
        <StyledModalTagsListItem><StyledModalTag>Fuel Consumption: {selectedCar?.fuelConsumption}</StyledModalTag>
        <svg width="2" height="16">
<use href={sprite + "#stick"} />
</svg>
        </StyledModalTagsListItem>
        <StyledModalTagsListItem><StyledModalTag>Engine Size: {selectedCar?.engineSize}</StyledModalTag>
        </StyledModalTagsListItem>
      </StyledModalTagsList>
      <StyledModalCarDesc>{selectedCar?.description}</StyledModalCarDesc>
      <StyledModalParagraph>Accessories and functionalities:</StyledModalParagraph>
      <StyledModalTagsList>
        {selectedCar?.functionalities.map((value, index, array) => <StyledModalTagsListItem><StyledModalTag>{value}</StyledModalTag>
      ...
      <div>
        <StyledModalCondList>
          {selectedCar?.rentalConditions
            .split(/(?=[A-Z])/)
            .map(rentCond => (
              <StyledModalCondListItem><StyledModalCondParagr>{rentCond}</StyledModalCondParagr></StyledModalCondListItem>
            ))}
        </StyledModalCondList>
